...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("C2CMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("C2CMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("C2CMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("olc-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("C2CMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running C2CMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadC2CMiner() exception");
    } catch (...) {
        LogPrintf("ThreadC2CMiner() exception");
    }

    LogPrintf("ThreadC2CMiner exiting\n");
}
